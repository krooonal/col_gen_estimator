...
class DTreeMasterProblemNew(DTreeMasterProblem):
    ...
    def solve_rmp(self, solver_params=''):
        ...
        return (leaf_duals, ns_duals)


class DTreeSubProblemNew(DTreeSubProblem):
    """TODO: Documentation."""

    def __init__(self, leaf, nodes, splits, targets,
                 depth, optimization_problem_type='cbc') -> None:
        super().__init__(leaf, nodes, splits, targets,
                         depth, optimization_problem_type)

    def create_submip(self, leaf_dual, ns_duals):
        """TODO: Documentation.
        """
        assert not self.generated_, "SP is already created."
        infinity = self.solver_.infinity()
    ...
    def update_objective(self, leaf_dual, ns_duals):
        """TODO: Documentation.
        """
        objective = self.solver_.Objective()
        n_rows = self.X_.shape[0]
    ...
    def generate_columns(self, X, y, dual_costs, params=""):
        ...
        if self.generated_:
            self.update_objective(leaf_dual, ns_duals)
        else:
            self.create_submip(leaf_dual, ns_duals)
...
class DTreeSubProblemHeuristicNew(DTreeSubProblemHeuristic):
    """TODO: Documentation."""
...
class DTreeClassifierNew(ColGenClassifier):
    """TODO: Documentation.
    """
    ...
    def __init__(self, initial_paths=[], leaves=[], nodes=[], splits=[],
        ...
        self.master_problem = DTreeMasterProblemNew(
            self.initial_paths, self.leaves, self.nodes, self.splits)
        self.subproblems = []
        all_subproblem_params = []
        heuristic = DTreeSubProblemHeuristicNew(
            self.leaves, self.nodes, self.splits, self.targets,
            self.tree_depth)
        self.subproblems.append([heuristic])
        all_subproblem_params.append([""])
